...
def find_files(path):
    ...
    print(f'\nFastq-files found: {fastq_files}. \nIdentifying single read and paired end reads files ...')
    ...
    for id, count in counts.items():
        if count == 2:
            ...
            single_read.append(id_no_direction_dict[id])
        else: print(f'Error for file {id}; could not determine whether file has single-reads or paired-end reads')
    
    print(f'Single-read files: {single_read}')
    print(f'\nLooking for paired-end reads and formatting ...\n')
...
def input_setup(input_path):
    ...
    formatted, single_read, comment = find_files(input_dir)
    print(comment)
    if 'Exiting' in comment:
        quit()
    
    return input_dir, formatted, single_read
...
def get_susceptibility_dicts(df_susceptibility):
    ...
    for drug in drug_list:
        ...
        for tool in tool_list:
            ...
            #print(f'tool {tool} met df_drug:')
            pprint.pprint(df_drug)
    ...
    print('hier komt overview_table')
    pprint.pprint(overview_table)
    return summary_table, overview_table
...
def MTBseq_join_amend(output_dir, sample):
    ...
    # Call MTBseq Amend module: creates phylo.fasta file in Amend folder
    ...
    log = subprocess.run(['conda', 'run', '-n', 'fasttree', 'fasttree', '-gtr', '-nt', fasta_path, '>', os.path.join(output_dir, sample, 'tree')], capture_output=True)
    with open(os.path.join(output_dir, sample, 'fasttree_log.txt'), 'w') as logfile:
        logfile.write(log.stdout.decode('utf-8'))
        logfile.write(log.stderr.decode('utf-8'))
...
input_dir, formatted_paired_end, single_read = input_setup(str(args.i))
...
for index, file in enumerate(formatted_paired_end):
    ...
    if str(args.mykrobe) == 'false': print('\tSkipped Mykrobe analysis')
    else: mykrobe_c, mykrobe_rc, mykrobe_stderr = mykrobe(input_dir, file, str(args.mykrobe), file_output_dir)
    if str(args.tbprofiler) == 'false': print('\tSkipped TBProfiler analysis')
    else: tbprofiler_c, tbprofiler_rc, tbprofiler_stderr = tbprofiler(input_dir, file, str(args.tbprofiler), file_output_dir)
    if str(args.gentb) == 'false': print('\tSkipped GenTB analysis')
    else: gentb_c, gentb_rc, gentb_stderr = gentb(input_dir, file, str(args.gentb), file_output_dir)
    if str(args.mtbseq) == 'false': print('\tSkipped MTBseq analysis')
    else: mtbseq_c, mtbseq_rc, mtbseq_stderr = mtbseq(input_dir, file, str(args.mtbseq), file_output_dir)

    
    # Read results files in file_output_dir
    res_tbprofiler, phylo_tbprofiler, qc_tbprofiler = tbprofiler_results(file[0], file_output_dir)
    res_mykrobe, phylo_mykrobe = mykrobe_results(file[0], file_output_dir)
    res_gentb = gentb_results(file[0], file_output_dir)
    res_mtbseq, phylo_mtbseq, qc_mtbseq = mtbseq_results(file[0], file_output_dir)
    ...
    summary_dict, overview_dict = get_susceptibility_dicts(df_susceptibility)
    ...
    #df_susceptibility.to_csv(os.path.join(output_dir, 'combined_sus_results.csv'), mode='a')
    ...
    qc_dict['mykrobe_c'] = mykrobe_c
    qc_dict['mykrobe_rc'] = mykrobe_rc
    qc_dict['mykrobe_stderr'] = mykrobe_stderr
    qc_dict['tbprofiler_v'] = tbprofiler_v
    qc_dict['tbprofiler_c'] = tbprofiler_c
    qc_dict['tbprofiler_rc'] = tbprofiler_rc
    qc_dict['tbprofiler_stderr'] = tbprofiler_stderr
    qc_dict['mtbseq_v'] = mtbseq_v
    qc_dict['mtbseq_c'] = mtbseq_c
    qc_dict['mtbseq_rc'] = mtbseq_rc
    qc_dict['mtbseq_stderr'] = mtbseq_stderr
    qc_dict['gentb_v'] = gentb_v
    qc_dict['gentb_c'] = gentb_c
    qc_dict['gentb_rc'] = gentb_rc
    ...
    MTBseq_join_amend(output_dir, file[0])
    ...
    print('finished!')
    #create_report(file[0], df_susceptibility)
